...
def _resolve_model_dir() -> Path:
    env_value = os.getenv(PKA_MODEL_DIR_ENV, "").strip()
    env_model_dir = Path(path.expanduser(path.expandvars(env_value))) if env_value else None
    candidates = []
    ...
    candidates.extend(
        [
            package_dir.parents[2] / "models" / "pka",
        ]
    )
...
def _model_checkpoint_paths() -> list[Path]:
    ...
    if missing:
        raise FileNotFoundError(
            "No se encontraron todos los modelos de pKa esperados: " + ", ".join(missing)
        )
    return model_paths
...
class QueryModel:
    def __init__(self):
        ...
        for checkpoint_path in _model_checkpoint_paths():
...
def _call_dimorphite_dl(
    ...
    try:
        with open(os.devnull, "w") as stderr_sink:
            sys.argv = [original_argv[0] if original_argv else "dimorphite_dl"]
